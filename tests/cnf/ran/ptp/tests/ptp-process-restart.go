		...
		It("should verify events are logged during sidecar recovery", reportxml.ID("84297"), func() {
			...
			for _, nodeInfo := range nodeInfoMap {
				...
				_, err = ptpdaemon.ExecuteCommandInPtpDaemonPod(RANConfig.Spoke1APIClient,
					nodeInfo.Name,
					killCommand,
					ptpdaemon.WithContainerName(ranparam.CloudEventProxyContainerName),
				...
				pluginTypes, err := profiles.GetPluginTypesFromProfile(gmProfile)
				Expect(err).ToNot(HaveOccurred(), "Failed to get plugin type for node %s", nodeInfo.Name)
				// for GM profiles, we expect only one plugin type (E810)
				Expect(pluginTypes).To(HaveLen(1), "Expected 1 plugin type for node %s", nodeInfo.Name)
				pluginType := pluginTypes[0]
				Expect(pluginType).To(Equal(ptp.PluginTypeE810), "Expected E810 plugin type for node %s", nodeInfo.Name)

				gmInterfaces, err := profiles.GetInterfacesWithPluginPins(gmProfile, pluginType, profiles.PinStateTx)
				Expect(err).ToNot(HaveOccurred(), "Failed to get GM interfaces with TX pin state for node %s", nodeInfo.Name)
				// This test assumes a single GM interface with TX pin state per profile.
				Expect(gmInterfaces).To(HaveLen(1), "Expected 1 GM interface with TX pin state for node %s",
					nodeInfo.Name)

				gmInterfaceName := gmInterfaces[0].GetNIC()
